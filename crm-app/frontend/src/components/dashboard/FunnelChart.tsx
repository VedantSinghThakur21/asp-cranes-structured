    ...
    <div className="w-full flex flex-col items-center" style={{ height: `${height}px` }}>
      {stages.map((stage, index) => {
        ...
        const widthPercentage =
          maxValue === 0 ? 40 : Math.max(40, Math.min(100, (stage.value / maxValue) * 100));

        // Calculate percentage of total
        const percentage = totalValue === 0 ? 0 : Math.round((stage.value / totalValue) * 100);

        // Determine position for styling
        const position = index === 0 ? 'first' : index === stages.length - 1 ? 'last' : 'middle';
