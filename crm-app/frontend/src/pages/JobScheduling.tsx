...
const TIME_SLOTS = Array.from({ length: 15 }, (_, i) =>
  addHours(new Date().setHours(6, 0, 0, 0), i)
);
...
export function JobScheduling() {
  ...
  useEffect(() => {
    if (formData.operatorId && formData.startDate && formData.endDate) {
      ...
      const isSelectedOperatorAvailable = availableOperators.some(
        op => op.id === formData.operatorId
      );
            ...
            quotations.forEach(quotation => {
              ...
              if (quotation.selectedEquipment && quotation.selectedEquipment.equipmentId) {
                ...
                const existingEquipment = equipment.find(
                  e => e.id === quotation.selectedEquipment.equipmentId
                );

                if (
                  existingEquipment &&
                  !dealSpecificEquipment.some(e => e.id === existingEquipment.id)
                ) {
                  ...
                  {days.map((day, dayIndex) => {
                    ...
                    const slotJobs = jobs.filter(job => {
                      ...
                      const matchesOperator =
                        !operatorFilter || job.operatorIds.includes(operatorFilter);
                      return (
                        matchesOperator &&
                        ...
                        {slotJobs.map((job, jobIndex) => (
                          ...
                          >
                            ...
                            <div className="text-xs text-black mt-1">
                              ...
                              {format(new Date(job.scheduledStartDate), 'h:mm a')} -{' '}
                              {format(new Date(job.scheduledEndDate), 'h:mm a')}
                            </div>
                          </div>
                        ))}
                      </div>
    ...
    <div className="space-y-6 font-sans">
      <div className="flex flex-wrap items-center mb-2 justify-between">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-lg font-semibold font-sans">{format(currentDate, 'MMMM yyyy')}</h2>
          <Button
            variant="outline"
            size="sm"
            onClick={() => setCurrentDate(prev => addDays(prev, 7))}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12 bg-gray-50 rounded-lg">
              ...
              <p className="text-gray-700 font-sans">
                Jobs can only be scheduled for deals that have been marked as "won". Return to the
                Deals page and move a deal to the "won" stage first.
              </p>
            </div>
          ) : (
            renderWeekView()
      ...
      <Modal
        ...
        title={
          wonDealId
            ? `Schedule Job for Won Deal: ${wonDealCustomerName || ''}`
            : 'Schedule Job for Won Deal'
        }
        size="lg"
      >
        <div className="space-y-6">
          ...
          {wonDealId && (
            <div className="bg-green-50 border border-green-200 p-4 rounded-md mb-4">
              ...
              <p className="text-green-700">
                ...
                {wonDealCustomerName || 'Unknown customer'}
              </p>
                  ...
                  try {
                    ...
                    if (quotations && quotations.length > 0) {
                      ...
                      quotations.forEach(quotation => {
                        // Handle single equipment
                        if (
                          quotation.selectedEquipment &&
                          quotation.selectedEquipment.equipmentId
                        ) {
                          const existingEquipment = equipment.find(
                            e => e.id === quotation.selectedEquipment.equipmentId
                          );
                          ...
                          if (
                            existingEquipment &&
                            !leadSpecificEquipment.some(e => e.id === existingEquipment.id)
                          ) {
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              {isLoadingEquipment ? (
                <p className="mt-1 text-xs text-blue-600">Loading equipment from quotations...</p>
              ) : dealEquipment.length > 0 ? (
                <p className="mt-1 text-xs text-green-600">
                  {dealEquipment.length} equipment item{dealEquipment.length !== 1 ? 's' : ''} from
                  deal quotation{dealEquipment.length !== 1 ? 's' : ''}
                </p>
              ) : formData.leadId ? (
                ...
                {(() => {
                  ...
                  return conflicts.length > 0 ? (
                      ...
                      <ul className="list-disc list-inside text-sm">
                        ...
                        {conflicts.map(conflict => (
                          <li key={conflict.id}>
                            ...
                            {format(new Date(conflict.scheduledStartDate), 'MMM d, h:mm a')}
                          </li>
                        ))}
                      </ul>
                    </div>
                    ...
                    <p className="text-success-600">
                      Equipment and operator are available for the selected time slot.
                    </p>
                  );
                })()}
              </div>
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <Button onClick={handleCreateJob} variant="accent">
            Schedule Job
          </Button>
        </div>
      </Modal>
      ...
      <Modal
        ...
        size="lg"
      >
        {selectedJob && <JobDetailsContent selectedJob={selectedJob} />}
      </Modal>
